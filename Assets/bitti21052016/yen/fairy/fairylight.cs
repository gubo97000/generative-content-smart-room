    ...
    public class fairylight : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (m_Burning)
            {
                ...
                float x = Mathf.PerlinNoise(m_Rnd + 3 + Time.time*2, m_Rnd + 4 + Time.time*4) - 0.5f;
                float y = Mathf.PerlinNoise(m_Rnd + 5 + Time.time*2, m_Rnd + 3 + Time.time*4) - 1.5f;
                float z = Mathf.PerlinNoise(m_Rnd + 3 + Time.time*2, m_Rnd + 4 + Time.time*4) - 0.5f;
